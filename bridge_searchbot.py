...
async def on_edit(event):
    ...
    if m.sender_id != BOT_ID:
        return
    if not m.text or not m.buttons:
        return
    ...
    if m.id in msg_map:
        ...
        try:
            await bot.edit_message(GRUPO, our_id, text=text[:4000], buttons=buttons)
            return
        except:
            pass
...
def ka():
    while True:
        time.sleep(600)
        try:
            urllib.request.urlopen(f"http://localhost:{int(os.environ.get('PORT', 10000))}", timeout=5)
        except:
            pass
