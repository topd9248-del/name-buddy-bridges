...
bot = TelegramClient('inv', API_ID, API_HASH, retry_delay=5, auto_reconnect=True, timeout=10)
...
async def free(event):
    ...
    async for m in bot.iter_participants(GRUPO_ID):
        if not m.bot and m.id != ADMIN_ID:
            try: await bot.edit_permissions(GRUPO_ID, m.id, send_messages=True)
            except: pass
