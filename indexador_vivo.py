import asyncio, pickle, json, os, re, hashlib
...
ADMIN_ID = 7771137226
INDEX_FILE = "video_index_vivo.pkl"
CANALES_FILE = "canales_vivo.json"
    ...
    try:
        ...
        async for m in user.iter_messages(ent, limit=None):
            if m.media and m.text:
                ...
                count += 1
                if count % 5000 == 0: guardar()
        if canal not in canales: canales.append(canal)
        guardar()
        await bot.send_message(ADMIN_ID, f"✅ {canal}: {count} títulos\n📊 Total: {len(title_index)}")
    except Exception as e:
        await bot.send_message(ADMIN_ID, f"❌ {canal}: {e}")

async def main():
    global bot, user
    bot = TelegramClient('vivo_bot', API_ID, API_HASH)
    ...
    print(f"✅ Indexador vivo - {len(title_index)} títulos")
